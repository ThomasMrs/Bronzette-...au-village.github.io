...
document.getElementById('year').textContent = new Date().getFullYear();
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Délégation d'événements : capte tout clic sur <a data-lightbox>
    document.body.addEventListener('click', (e) => {
      const link = e.target.closest('a[data-lightbox]');
      if (!link) return;

      e.preventDefault(); // empêche l’ouverture du lien
      const cardTitle = link.closest('.card')?.querySelector('h4')?.textContent?.trim() || '';
      openLightbox(link.getAttribute('href'), cardTitle);
    });
